...
async def aarlo_snapshot_service_handler(camera, _service):
    ...
    hass.bus.fire(
        ...
        {
            "entity_id": "camera.aarlo_" + camera.unique_id,
        },
    )
...
async def aarlo_snapshot_to_file_service_handler(camera, service):
    ...
    try:
        ...
        hass.bus.fire(
            "aarlo_snapshot_ready",
            {"entity_id": "camera.aarlo_" + camera.unique_id, "file": snapshot_file},
        )
...
async def aarlo_video_to_file_service_handler(camera, service):
    ...
    try:
        ...
        hass.bus.fire(
            "aarlo_video_ready",
            {"entity_id": "camera.aarlo_" + camera.unique_id, "file": video_file},
        )
    except OSError as err:
        _LOGGER.error("Can't write image to file: %s", err)
